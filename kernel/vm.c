...
kvminit()
{
  kernel_pagetable = kvminit_helper();
  // CLINT
...
// Recursively free page-table pages.
// All leaf mappings must already is not required
void
...
// 修改自uvmcopy
int
kvmcopy_mappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
